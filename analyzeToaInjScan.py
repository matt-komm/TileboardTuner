...
for filePath,addParams in autodetectFiles(
    "/home/mkomm/Analysis/HGCAL/cerntestbeam/2024-08-01_17-32-26_ConvGain4_scan_Calib_2V5_ch0_newphase",
...
#dfTotSel = dfTot[(dfTot['chip']==0)&(dfTot['half']==0)&(dfTot['channel']==0)]


dfToaFired = dfTot.groupby(['real_channel','Calib_2V5','channel','half','chip'],as_index=False ).agg({'toa_fired': 'sum'})
print (dfToaFired[(dfToaFired['chip']==0)&(dfToaFired['half']==0)&(dfToaFired['channel']==0)])
...
for chip in dfTot['chip'].unique():
    for half in dfTot['half'].unique():
        ...
        plt.xlim([0,100])
        plt.savefig(f"chip{chip}_half{half}_channel{channel}_toacinj.png")
        plt.close()
